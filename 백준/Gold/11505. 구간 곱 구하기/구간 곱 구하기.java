...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0 ; i < M + K ; i++) {
            ...
            if (a == 1) {
                ...
                sb.append(getNum(1, N, 1, b, (int)c) % DIV).append("\n");
        }
    ...
    static long init(int start, int end, int node) {
        if (start == end)
            return tree[node] = arr[start] % DIV;
    ...
    static long getNum(int start, int end, int node, int left, int right) {
        ...
        return (getNum(start, mid, node * 2, left, right) * getNum(mid + 1, end, node * 2 + 1, left, right) ) % DIV;
    }
